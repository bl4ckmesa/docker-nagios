...
RUN apt-get update && apt-get install -y iputils-ping netcat build-essential snmp snmpd snmp-mibs-downloader php5-cli apache2 libapache2-mod-php5 runit bc postfix bsd-mailx libdbi-perl libdbd-mysql-perl libnagios-plugin-perl libssl-dev mysql-client vim dnsutils libnet-snmp-perl python-redis
...
RUN ( cd /usr/local ** ln -s /opt/nagios && cd /usr/lib && ln -s x86_64-linux-gnu/libssl.so libssl.so.6 && ln -s x86_64-linux-gnu/libcrypto.so libcrypto.so.6 && ln -s x86_64-linux-gnu/libmysqlclient.so.18 libmysqlclient.so.15 )
